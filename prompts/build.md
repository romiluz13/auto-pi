...
1. **Build.** Use `/skill:implement` as the execution wrapper if a spec exists. Otherwise implement directly. `/skill:implement` drives `/skill:tdd` internally — one red-green slice at a time. Follow existing patterns in the codebase. Don't over-engineer — three similar lines beats a premature helper. Python → use `/skill:uv`. Fix any type/LSP errors immediately when detected.
