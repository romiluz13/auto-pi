...
1. **Build.** Use `/implement` as the execution wrapper if a spec exists. Otherwise implement directly. Follow existing patterns in the codebase. Don't over-engineer — three similar lines beats a premature helper. Python → use `uv`. Fix any type/LSP errors immediately when detected.
