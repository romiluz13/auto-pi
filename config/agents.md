...
- **Web** (8): `search`, `scrape`, `discover-api`, `data-feeds`, `live-research`, `agent-browser`, `rag-pipeline`, `brightdata-cli` — auto-trigger for web tasks. Use `bdata` CLI for SERP/scrape/structured data. Use `pi-web-access` for YouTube video understanding, PDF extraction, and local video analysis. `bright-data-best-practices` is a model-invoked reference for BD APIs.
