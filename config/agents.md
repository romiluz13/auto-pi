...
**Enforcement gates** <!-- scar: 2026-07-16 — audit found model writes code without TDD and commits without review; prominence tier (~90%) was not enough -->: The workflow-gate extension now BLOCKS operations, not just warns: (1) `write`/`edit` on source files is blocked unless `tdd` is active — test files and docs are always allowed; (2) `git commit` is blocked unless `code-review` was loaded AND `verification-before-completion` was loaded or tests ran; (3) `git push` is blocked unless `commit` was loaded. Escape hatch: `/skip-gate` toggles all gates for the session. Use it for quick edits or intentional skips. These are enforcement (100% non-circumventable), not prominence (~90%). If a gate blocks you, do NOT work around it by reading the skill file from disk — run the slash command or use `/skip-gate` explicitly.
